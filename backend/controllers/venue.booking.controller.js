...
export const getAllVenueBookings = asyncHandler(async (req, res, next) => {
  ...
  const bookings = await VenueBooking.find().populate("venueId categoryId organizerId", "name");
  return successResponse({ res, data: { bookings } });
});

// =================== Get By Organizer ====================
export const getVenueBookingsByOrganizer = asyncHandler(async (req, res) => {
  const bookings = await VenueBooking.find({ organizerId: req.user._id }).populate("venueId categoryId", "name");
  return successResponse({ res, data: { bookings } });
});
