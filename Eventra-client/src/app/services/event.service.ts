...
export class EventService {
  ...
  getMyEvents(
    categoryId?: string
  ): Observable<{ message: string; data: { events: Event[] } }> {
    let params = new HttpParams();
  ...
  getEventById(
    id: string
  ): Observable<{ message: string; data: { event: Event } }> {
  ...
  ): Observable<{ message: string; data: Event }> {
    return this.http.post<{ message: string; data: Event }>(this.apiUrl, event);
  }
